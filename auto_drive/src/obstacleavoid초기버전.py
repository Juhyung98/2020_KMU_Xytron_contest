...
def obstacle_avoiding(image) :
	global lidar_scan

	real_center, copy_image = process_image(image)
	left_point =[]
	right_point =[]
	...
	if right_average >  left_average :
		angle = -30 #-(calculate_pid (Width/3, real_center,5.5,0.1,1.1) / 12.0)
		Direction = direction.right
	else:
		angle = 30 #-(calculate_pid (Width*2/3, real_center,5.5,0.1,1.1) / 12.0)
		Direction = direction.left
	print(angle)

	if np.all(lidar_scan[60:120] >= 0.5) :
...
def start():
	...
	while True:
		...
		real_center, copy_image = process_image(image)
		angle = -(calculate_pid (Width/2, real_center,3.0,0.05,1.3) / 12.0) # 5.5,0.1,1.1 //17.0    next time input this 3.5 0.1 1.1 !!!!!
		...
		while(State == drive_state.obstacle):	
			State, angle = obstacle_avoiding(image)
			drive(angle, 1)
