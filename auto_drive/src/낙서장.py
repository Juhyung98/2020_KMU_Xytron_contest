...
def process_image(frame):
	...
	# blur
	kernel_size = 5
	blur_gray = cv2.GaussianBlur(frame,(kernel_size, kernel_size), 1)


	mark=np.copy(blur_gray)
	bgr_threshold = [190,190,190]
	...
	low_threshold = 50
	high_threshold = 150
	edge_img = cv2.Canny(np.uint8(mark), low_threshold, high_threshold)
...
def hsv_process(frame):
	...
	# blur
	kernel_size = 5
	blur_hsv = cv2.GaussianBlur(hsv, (kernel_size, kernel_size), 10)
	...
	#canny
	canny_hsv = cv2.Canny(np.uint8(mask), 50, 150)
...
def start():
	...
	while True:
		...
		#hsv_image = hsv_process(image)
		#cv2.imshow('hsv_edge', hsv_image)
		...
		if(State == drive_state.drive):
			...
			if obstacle_finish == True and detect_across == False and detect_cross == False:#한번 감지시 더이상 반복 X
				...
				if cnt > across_bar and time.time()-obstacle_duration > 0.9:#across_bar is 18 now
					detect_across = True
					speed = 3
