...
class BirdEyeView() :
	def __init__(self, img) :
		...
		# self.__src = np.float32([[-50, self.img_h], [195, 280], [460, 280], [self.img_w+50 , self.img_h]]) ## 원본이미지의 warping 포인트
		# self.__dst = np.float32([[0,480] , [0,0] , [640, 0],[640,480]]) ## 결과 이미지에서 src가 매칭될 점들
...
def get_errorControl_speed(valueForCentering):
		...
		desired_speedForCentering = 0 #valueForCentering
...
def calculate_PID(error, errorControl):
	...
	kp = 5.0                                               ## golden ratio kp for P control
	ki = 1.0 	   								## golden ratio ki for I control                         ###
	kd = 3.0                                                ## golden ratio kd for D control
	#constant_PresaturatedToAngle =  float(1) / 46           ## constant_PresaturatedToAngle size is related with kp
...
def start():
	...
	while True:
		...
		#center = (lpos2 + rpos2) / 2 - (lpos + rpos) / 2  
		center = (lpos + rpos) / 2
		
		middle_x = -(Width/2 - center)
		#temp_middle_x = middle_x
		#print(middle_x)
		#pid control
		valueForCentering = center
