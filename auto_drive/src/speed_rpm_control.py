...
def speed_callback(msg):
	global speed_rpm
	speed_rpm = msg / 440 # if speed max 5000 constant will be 440
	print("speed : ", msg)
...
WEIGHT = 700
LIMIT_ANGLE = 50
n_win = 35 # 좌,우 차선별 탐지 윈도우의 개수, 적어지면 샘플링이 적어지는 샘이라서 급커브 같은데서 영역을 정확히 못잡아냄
...
def get_errorControl_speed(valueForCentering):
		global error_speed, errorList_speed, speed_box, error_i_speed       ## record error
	
		desired_speedForCentering = valueForCentering #valueForCentering
        print(speed_box)
		error_speed = desired_speedForCentering - speed_box
        error_i_speed = error_i_speed + error_speed
		#print(error_speed)
...
def calculate_PID_speed(error_speed, errorControl_speed):
	global constant_PresaturatedToSpeed, presaturated_output_speed,error_i_speed
	...
	integral_output = ki * error_i_speed                                   ###
...
def start():
	...
	global cap
	global Width, Height
	global error, errorList, error_speed, errorList_speed, speed_box
	...
	while True:
		...
		drive(presaturated_output, presaturated_output_speed)
