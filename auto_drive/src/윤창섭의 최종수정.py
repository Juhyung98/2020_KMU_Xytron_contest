...
def go_left(real_center) :
	global lidar_scan
	angle = (calculate_pid (Width/10*3, real_center,3.0,0.1,1.1) / 17.0)
...
def go_right(real_center) :
	global lidar_scan
	angle = (calculate_pid (Width/10*7, real_center,3.0,0.1,1.1) / 17.0)
...
def start():
	...
	while True:
		...
		if(State == drive_state.drive):
			angle = -(calculate_pid (Width/2, real_center,3.0,0.05,1.3) / 9.0) # 5.5,0.1,1.1 //17.0    next time input this 3.5 0.1 1.1 !!!!!
