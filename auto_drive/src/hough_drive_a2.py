...
def obstacle_avoiding(image) :
	...
	if right_average >  left_average :
		angle = -35 #-(calculate_pid (Width/3, real_center,5.5,0.1,1.1) / 12.0)
		Direction = direction.right
	else:
		angle = 35 #-(calculate_pid (Width*2/3, real_center,5.5,0.1,1.1) / 12.0)
		Direction = direction.left
	print(angle)

	if np.all(lidar_scan[70:110] >= 0.5) :
...
def start():
	...
	while True:
		...
		real_center, center, copy_image = process_image(image)
		angle = -(calculate_pid (Width/2, center,3.5,0.01,1.1) / 10.0) # 5.5,0.1,1.1 //17.0    next time input this 3.5 0.1 1.1 !!!!!
		...
		speed = (calculate_pid(30,abs(angle),3.0,0.3,0.5)/3) # set value more precisely 
		if speed <= 2.0 :
			speed = 2.0
